...
grid <- expand.grid(n = Dimensions, L = side_lengths, KEEP.OUT.ATTRS = FALSE)

results <- do.call(rbind, lapply(seq_len(nrow(grid)), function(i) {
  n <- grid$n[i]; L <- grid$L[i]
  ...
  gap <- clusGap(dat$X, FUNcluster = FUNcluster, K.max = n + 3, B = 20)
  best_k <- maxSE(gap$Tab[, "gap"], gap$Tab[, "SE.sim"], method = "Tibs2001SEmax")

  cat("Finished n =", n, "L =", L, "-> k̂ =", best_k, "\n")
  data.frame(Dimension = n, SideLength = L, EstimatedK = best_k, TrueK = n)
}))

write.csv(results, file.path(Directory, "results", "task1_results.csv"), row.names = FALSE)
```
